...
try {
    ...
    if ($projects.Count -ne 8) {
        throw "Expected exactly eight FoundationKit package projects, found $($projects.Count)."
    }
    ...
    if ($packages.Count -ne 8 -or $symbols.Count -ne 8) {
        throw "Expected eight packages and eight symbol packages."
    }

    Write-Host "Created 8 packages and 8 symbol packages in $Output"
}
finally {
    Pop-Location
}
