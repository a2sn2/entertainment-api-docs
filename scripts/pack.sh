...
if [[ ${#projects[@]} -ne 8 ]]; then
  echo "Expected exactly eight FoundationKit package projects, found ${#projects[@]}." >&2
  exit 1
fi
...
if [[ "$package_count" -ne 8 || "$symbol_count" -ne 8 ]]; then
  echo "Expected eight packages and eight symbol packages; got $package_count and $symbol_count." >&2
  exit 1
fi
