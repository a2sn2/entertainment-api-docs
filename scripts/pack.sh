...
if [[ ${#projects[@]} -ne 7 ]]; then
  echo "Expected exactly seven FoundationKit package projects, found ${#projects[@]}." >&2
  exit 1
fi
...
if [[ "$package_count" -ne 7 || "$symbol_count" -ne 7 ]]; then
  echo "Expected seven packages and seven symbol packages; got $package_count and $symbol_count." >&2
  exit 1
fi
