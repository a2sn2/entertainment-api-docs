...
if [[ ${#projects[@]} -ne 17 ]]; then
  echo "Expected exactly seventeen FoundationKit package projects, found ${#projects[@]}." >&2
  exit 1
fi
...
if [[ "$package_count" -ne 17 || "$symbol_count" -ne 17 ]]; then
  echo "Expected seventeen packages and seventeen symbol packages; got $package_count and $symbol_count." >&2
  exit 1
fi
