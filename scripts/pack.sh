...
if [[ ${#projects[@]} -ne 11 ]]; then
  echo "Expected exactly eleven FoundationKit package projects, found ${#projects[@]}." >&2
  exit 1
fi
...
if [[ "$package_count" -ne 11 || "$symbol_count" -ne 11 ]]; then
  echo "Expected eleven packages and eleven symbol packages; got $package_count and $symbol_count." >&2
  exit 1
fi
