...
validation="$(printf '%s\n' "$validation_raw" | tr -d '\r' | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//' -e '/^$/d')"

printf '%s\n' "$validation"
for prefix in users initiatives reviews audit; do
  if ! grep -Eq "^${prefix}=[0-9]+$" <<< "$validation"; then
    echo "Restore validation did not return a numeric ${prefix} count." >&2
    exit 1
  fi
done
