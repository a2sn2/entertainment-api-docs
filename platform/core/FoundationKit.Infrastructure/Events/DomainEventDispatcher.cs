        ...
        CancellationToken cancellationToken = default)
    {
        foreach (var domainEvent in domainEvents)
        {
            ...
            var handleMethod = handlerType.GetMethod(nameof(IDomainEventHandler<IDomainEvent>.HandleAsync))
                ?? throw new InvalidOperationException($"Handler method was not found for {handlerType.Name}.");

            foreach (var handler in serviceProvider.GetServices(handlerType))
            {
                var task = handleMethod.Invoke(handler, [domainEvent, cancellationToken]) as Task
                    ?? throw new InvalidOperationException($"{handlerType.Name} returned an invalid task.");
