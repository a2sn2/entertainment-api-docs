...
public sealed class DocumentRepository(AppDbContext dbContext)
    : EfRepository<DocumentationDocument, Guid, AppDbContext>(dbContext), IDocumentRepository
{
    public Task<bool> ReferenceExistsAsync(string reference, CancellationToken cancellationToken) =>
        dbContext.Documents.AnyAsync(document => document.Reference == reference, cancellationToken);

    public Task<bool> SlugExistsAsync(string slug, CancellationToken cancellationToken) =>
        dbContext.Documents.AnyAsync(document => document.Slug == slug.ToLowerInvariant(), cancellationToken);
    ...
    public Task AddVersionAsync(DocumentVersion version, CancellationToken cancellationToken) =>
        dbContext.DocumentVersions.AddAsync(version, cancellationToken).AsTask();
}
