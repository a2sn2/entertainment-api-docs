...
| R-FK-002 | Malicious/vulnerable dependency or secret enters supply chain | Source, packages, images | Central versions, `.gitignore`, build/test | High | NuGet audit, secret scan, SBOM/inventory, action pinning | CI security job |
...
| R-FK-009 | Secrets leak via local files/terminal/logs | Admin/SQL credentials | `.gitignore`, generated values | High | Stop routine printing, restrictive local storage guidance, secret scanning | Script tests + scan |
